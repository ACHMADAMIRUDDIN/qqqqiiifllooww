...
<body>
    ...
    <footer>
        ...
        <!-- Chat Box -->
        <div id="chatBox"
            style="display:none;position:fixed;bottom:90px;right:30px;z-index:1000;width:320px;max-width:95vw;background:#fff;border-radius:16px;box-shadow:0 4px 24px #0003;overflow:hidden;flex-direction:column;">
            <!-- Header -->
            <div
                style="background:#2563eb;color:#fff;padding:1em 1.2em;font-weight:600;display:flex;justify-content:space-between;align-items:center;">
                <span>Chat Admin</span>
                <button onclick="toggleChat()"
                    style="background:none;border:none;color:#fff;font-size:1.3em;cursor:pointer;">&times;</button>
            </div>
            <!-- Messages -->
            <div id="chat-messages"
                style="height:220px;overflow-y:auto;padding:1em;background:#f8fafc;display:flex;flex-direction:column;gap:10px;font-size:14px;">
                <!-- Contoh pesan -->
    ...
    <div style="align-self:flex-end;background:#2563eb;color:#fff;padding:8px 12px;border-radius:12px 12px 0 12px;max-width:80%;">Ya, saya ingin tanya...</div> -->
        ...
        <script>
            ...
            function renderPasienDropdown() {
                if (!isAdmin) return;
                let dropdown =
                    `<select id="pasien-dropdown" style="width:100%;margin-bottom:10px;padding:6px 10px;border-radius:8px;border:1px solid #ccc;">`;
                dropdown += `<option value="">Pilih Pasien</option>`;
                @foreach ($pasiens as $pasien)
                    dropdown +=
                        `<option value="{{ $pasien->id }}">{{ addslashes($pasien->name) }} ({{ $pasien->email }})</option>`;
                @endforeach
                dropdown += `</select>`;
            ...
            function loadChat() {
                if (!selectedUserId) {
                    if (isAdmin) {
                        document.getElementById('chat-messages').innerHTML =
                            '<div style="color:#888;text-align:center;">Pilih pasien untuk mulai chat</div>';
                    }
                ...
                fetch(`/chat/messages/${selectedUserId}`)
                    ...
                    .catch(() => {
                        document.getElementById('chat-messages').innerHTML =
                            '<div style="color:#c00;text-align:center;">Gagal memuat pesan. Hubungi admin.</div>';
                    });
            }
